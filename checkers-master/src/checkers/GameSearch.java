    ...
    public static Board minimaxAB(Board board, int recursion, int player, Board alpha, Board beta) 
    {
        if (recursion > 0) 
        {
            ...
            {   
            	...
            	while (iterator.hasNext()) 
                {
                    Board nextBoard = minimaxAB(executeMove(iterator.next(), board), recursion - 1, opponent(player), alpha, beta);
    ...
    public static Board minimax(Board board, int recursion, int player) 
    {
        if (recursion > 0) 
        {
            ...
            {   
                ...
                while (iterator.hasNext()) 
                {
                    boardlist.add(minimax(executeMove(iterator.next(), board), recursion - 1, opponent(player)));
                }
...
*/
    ...
    {
        return validCoordinate(from) && validCoordinate(to) && board.vacantCoordinate(to) && !board.vacantCoordinate(from) && (from.row() - to.row() == 1) && ((from.upLeftMove().equals(to)) || (from.upRightMove().equals(to)));
    }

    private static boolean validDownMove(Coordinate from, Coordinate to, Board board) 
    {
        return validCoordinate(from) && validCoordinate(to) && board.vacantCoordinate(to) && !board.vacantCoordinate(from) && (to.row() - from.row() == 1) && ((from.downLeftMove().equals(to)) || (from.downRightMove().equals(to)));
    }

    private static boolean validUpJump(Coordinate from, Coordinate to, int color, Board board) 
    {
        boolean basis = validCoordinate(from) && validCoordinate(to) && board.vacantCoordinate(to) && !board.vacantCoordinate(from) && (board.getChecker(from).getColor() == color) && (from.row() - to.row() == 2);
        
        if (basis) 
        {
            if (from.upLeftJump().equals(to)) 
            {
                return (!board.vacantCoordinate(from.upLeftMove()) && (board.getChecker(from.upLeftMove()).getColor() == opponent(color)));
            } 
            
            else if (from.upRightJump().equals(to)) 
            {
                return (!board.vacantCoordinate(from.upRightMove()) && (board.getChecker(from.upRightMove()).getColor() == opponent(color)));
            } 
    ...
    {
        boolean basis = validCoordinate(from) && validCoordinate(to) && board.vacantCoordinate(to) && !board.vacantCoordinate(from) && (board.getChecker(from).getColor() == color) && (to.row() - from.row() == 2);
        
        if (basis) 
        {
            if (from.downLeftJump().equals(to)) 
            {
                return (!board.vacantCoordinate(from.downLeftMove()) && (board.getChecker(from.downLeftMove()).getColor() == opponent(color)));
            } 
            
            else if (from.downRightJump().equals(to)) 
            {
                return (!board.vacantCoordinate(from.downRightMove()) && (board.getChecker(from.downRightMove()).getColor() == opponent(color)));
            } 
